...
/* ================= Trip Records (ID card style) ================= */
const tripForm = document.getElementById('tripForm');
tripForm.addEventListener('submit', async (e) => {
  e.preventDefault();
  const record = {
    tour_name: tripForm.tourName.value.trim(),
    tour_date: tripForm.tourDate.value || null,
    members_count: parseInt(tripForm.membersCount.value) || 0,
    description: tripForm.description.value.trim(),
    image_url: tripForm.imageUrl.value.trim()
  };
  const { error } = await db.from('trip_records').insert([record]);
  if (error) { showToast('Error: ' + error.message); return; }
  showToast('Trip record save ho gaya.');
  tripForm.reset();
  loadTripRecords();
});
...
async function loadTripRecords() {
  const el = document.getElementById('tripAdminList');
  if (!el) return;
  el.innerHTML = '<div class="empty-state"><div class="spin" style="margin:0 auto"></div></div>';
  const { data, error } = await db.from('trip_records').select('*').order('tour_date', { ascending: false });
  if (error) { el.innerHTML = `<div class="empty-state">Load nahi ho saka: ${error.message}</div>`; return; }
  if (!data || data.length === 0) { el.innerHTML = '<div class="empty-state">Abhi tak koi trip record nahi hai. Upar form se add karein.</div>'; return; }
  el.innerHTML = '';
  data.forEach(t => {
    const card = document.createElement('div');
    card.className = 'id-card';
    card.innerHTML = `
      <div class="id-top"><b>Alvi Tours & Travels</b><span style="font-size:.72rem;">Trip Record</span></div>
      ${t.image_url ? `<div class="id-photo"><img src="${t.image_url}" alt=""></div>` : ''}
      <div class="id-badge">${t.members_count || 0}<span>Log</span></div>
      <div class="id-body">
        <h4>${t.tour_name}</h4>
        <div class="id-meta"><span>${fmtDate(t.tour_date)}</span></div>
        <div class="id-desc">${t.description || ''}</div>
      </div>
      <div class="id-actions">
        <button class="btn btn-teal btn-sm" data-act="notice">Notice Board Par Dikhayein</button>
        <button class="btn btn-sm" style="background:var(--maroon);color:#fff;" data-act="del">Delete</button>
      </div>`;
    card.querySelector('[data-act="notice"]').addEventListener('click', async () => {
      const { error } = await db.from('notices').insert([{
        title: t.tour_name,
        message: `${t.members_count || 0} log is tour par gaye — ${t.description || ''}`.trim(),
        image_url: t.image_url || ''
      }]);
      showToast(error ? 'Error: ' + error.message : 'Notice board par post ho gaya.');
      loadAdminNotices();
    });
    card.querySelector('[data-act="del"]').addEventListener('click', async () => {
      if (confirm('Yeh trip record delete karna hai?')) {
        await db.from('trip_records').delete().eq('id', t.id);
        loadTripRecords();
      }
    });
    el.appendChild(card);
